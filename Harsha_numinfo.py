...
HF_INDEX_BASE = os.environ.get(
    "ICMR_HF_INDEX_BASE",
    "https://huggingface.co/datasets/MRSHREY197/icrm-hitek-full-db-mixed/resolve/main",
).rstrip("/")
...
DEVELOPER_NAME = "HARSHA VARDHAN RAJU"
...
REMOTE_INDEXES = {
    "phone": [
        f"{HF_INDEX_BASE}/idx_phone.{i}.parquet"
        for i in range(7)
    ],
    "aadhar": [
        f"{HF_INDEX_BASE}/idx_aadhar.{i}.parquet"
        for i in range(7)
    ],
}
...
def _new_conn() -> duckdb.DuckDBPyConnection:
    ...
    for kind, urls in REMOTE_INDEXES.items():
        ...
        con.execute(
            ...
            f"AS SELECT * FROM read_parquet([{lst}])"
        )
...
def root():
    return {
        ...
        "docs": "/docs",
        "developer_channel": "Harsha_vardha_raju",
        "channel": "HIDDEN_HACKERS",
    }
...
def search_ui(query: str, limit: int) -> str:
    ...
    if not query or not query.strip():
        return (
            "⚠️ Kuch toh search karo — "
            "phone, aadhar, ya name daalo."
        )

    q = query.strip()

    try:
        data = _unified_search(
            q,
            int(limit),
        )
...
def build_ui():
    ...
    with gr.Blocks(
        ...
        gr.Markdown(
            "Search **2.5 billion records** — "
            "phone, Aadhaar, name, address & more",
...
**Source:** [HF Dataset](https://huggingface.co/datasets/MRSHREY197/icrm-hitek-full-db-mixed)
                """
            )
